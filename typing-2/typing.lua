...
function typing:draw()
	local display_text = self:scroll_along_limiter(self.text)
	-- Box drawing
	...
	love.graphics.setColor(1,1,1,1)
	-- Prints the text
	love.graphics.printf(self.text, self.x+1, self.y, love.graphics.getWidth())
...
function typing:text_insert(str)
	self.text = self.text:sub(1, math.min(cursor_letter_index, selected_text_i))..str..self.text:sub(math.max(cursor_letter_index+1, selected_text_i+1), self.text:len()+1)
